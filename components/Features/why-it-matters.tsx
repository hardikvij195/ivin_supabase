...
export default function WhyItMatters() {
  ...
  // ✅ only valid when VIN length is exactly 17
    ...
    <div className="layout flex justify-between flex-col md:flex-row xl:px-20 px-8 xl:pt-16 pt-10 gap-10 md:gap-24">
      <div className="flex-1 max-w-150">
        ...
        <p className="text-[#4D4D4D] text-[20px] pb-6">
          ...
          you sell with trust. vinX’s detailed vehicle history reports reduce
          risk, uncover hidden problems, and ensure you're making the most
          informed decision possible.
        </p>
        <p className="text-[24px] text-[#1C1C1C] font-semibold mb-4">Ready to Try vinX?</p>
        ...
        <div className="w-full flex flex-col sm:flex-row justify-between gap-[12px] mb-8">
          <input
            ...
            maxLength={17} // ✅ limit input to 17 chars
            className="bg-[#FAFAFA] text-[#999999] px-8 py-4 border border-[#E4E4E4] rounded-full w-full"
            placeholder="e.g. 1HGCM82633A004352"
          />
          <button
            onClick={handleGetReports}
            disabled={!isVinValid} // ✅ disable until exactly 17 chars
            className="px-10 py-5 font-fredoka bg-primary text-white font-semibold rounded-full w-max whitespace-nowrap disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Get Reports
          </button>
        </div>
